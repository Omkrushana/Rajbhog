export const restaurants = [
  {
    imageUrl:
      "https://th.bing.com/th/id/OIP.jY7cjlPmytF1wEGvHIO4NQHaE8?rs=1&pid=ImgDetMain",
    name: "Delicious Bites",
    rating: 4.5,
    city: "New York",
    typeOfFood: "Italian",
    description:
      "Delicious Bites offers a sophisticated Italian dining experience in the heart of New York. Indulge in a variety of classic Italian dishes prepared with the finest ingredients.",
  },
  {
    imageUrl:
      "https://www.sizzlingpubs.co.uk/content/dam/sizzling-pubs/images/2022/refresh/sizzling-guarantee/sizzling-offers-sizzlingguarantee-banner-nocta.jpg.asset/1644405284327.jpg",
    name: "Sizzling Grill",
    rating: 4.2,
    city: "Los Angeles",
    typeOfFood: "Steakhouse",
    description:
      "Sizzling Grill is a renowned steakhouse in Los Angeles, where every dish is a celebration of flavors. Our expert chefs grill the finest cuts to perfection for an unforgettable dining experience.",
  },
  {
    imageUrl:
      "https://th.bing.com/th/id/OIP.Z_f9YCwW_gSAKc6BTF6GwwHaCF?rs=1&pid=ImgDetMain",
    name: "Spice Paradise",
    rating: 4.7,
    city: "Chicago",
    typeOfFood: "Indian",
    description:
      "Step into Spice Paradise and embark on a culinary journey through the vibrant flavors of India. Our menu features a rich selection of aromatic spices and authentic Indian dishes.",
  },
  {
    imageUrl:
      "https://th.bing.com/th/id/OIP.X1XnAdh3GjJ8_mYnJOYx6gHaFj?rs=1&pid=ImgDetMain",
    name: "Mediterranean Delight",
    rating: 4.0,
    city: "San Francisco",
    typeOfFood: "Mediterranean",
    description:
      "Mediterranean Delight offers a taste of the sun-drenched flavors of the Mediterranean. Enjoy a culinary journey with our diverse menu inspired by the coastal cuisines of the region.",
  },
  {
    imageUrl:
      "https://th.bing.com/th/id/OIP.9c-p37sU8EavDkgo_NgCzgAAAA?rs=1&pid=ImgDetMain",
    name: "Tokyo Fusion",
    rating: 4.8,
    city: "Tokyo",
    typeOfFood: "Japanese",
    description:
      "Experience the art of Japanese cuisine at Tokyo Fusion. Our chefs craft each dish with precision and creativity, bringing the authentic flavors of Tokyo to your table.",
  },
  {
    imageUrl: "https://example.com/restaurant6.jpg",
    name: "Cafe Parisienne",
    rating: 4.4,
    city: "Paris",
    typeOfFood: "French",
    description:
      "Transport yourself to the charming streets of Paris at Cafe Parisienne. Indulge in the finest French cuisine and savor the romantic atmosphere of a classic Parisian cafe.",
  },
  {
    imageUrl: "https://example.com/restaurant7.jpg",
    name: "Taco Haven",
    rating: 4.1,
    city: "Mexico City",
    typeOfFood: "Mexican",
    description:
      "Taco Haven brings the vibrant and bold flavors of Mexico City to your plate. Enjoy a fiesta of tacos, enchiladas, and other Mexican delights in a lively atmosphere.",
  },
  {
    imageUrl: "https://example.com/restaurant8.jpg",
    name: "Zen Sushi",
    rating: 4.6,
    city: "Kyoto",
    typeOfFood: "Sushi",
    description:
      "Embark on a culinary journey through the flavors of Japan at Zen Sushi. Our expert sushi chefs craft exquisite rolls and sashimi, offering a taste of traditional Japanese artistry.",
  },
  {
    imageUrl: "https://example.com/restaurant9.jpg",
    name: "BBQ Bonanza",
    rating: 4.3,
    city: "Austin",
    typeOfFood: "Barbecue",
    description:
      "BBQ Bonanza is a haven for barbecue enthusiasts. Indulge in smoky, flavorful meats slow-cooked to perfection, and experience the true essence of Texas-style barbecue.",
  },
  {
    imageUrl: "https://example.com/restaurant10.jpg",
    name: "Greek Isles Taverna",
    rating: 4.5,
    city: "Athens",
    typeOfFood: "Greek",
    description:
      "Transport yourself to the picturesque Greek Isles at Greek Isles Taverna. Savor the authentic flavors of Greece with our traditional dishes and warm Mediterranean ambiance.",
  },
  {
    imageUrl: "https://example.com/restaurant11.jpg",
    name: "Seoul Spice House",
    rating: 4.7,
    city: "Seoul",
    typeOfFood: "Korean",
    description:
      "Seoul Spice House invites you to experience the bold and savory flavors of Korean cuisine. Enjoy a fusion of traditional and modern Korean dishes in a vibrant setting.",
  },
  {
    imageUrl: "https://example.com/restaurant12.jpg",
    name: "Rustic Pizzeria",
    rating: 4.2,
    city: "Rome",
    typeOfFood: "Pizza",
    description:
      "Rustic Pizzeria captures the essence of authentic Italian pizza. Our wood-fired oven creates crispy crusts topped with the finest ingredients, delivering a taste of Rome in every bite.",
  },
  {
    imageUrl: "https://example.com/restaurant13.jpg",
    name: "Vegan Delights",
    rating: 4.6,
    city: "Berlin",
    typeOfFood: "Vegan",
    description:
      "Vegan Delights is a haven for plant-based enthusiasts. Our menu features creative and flavorful vegan dishes that showcase the diversity and deliciousness of plant-based cuisine.",
  },
  {
    imageUrl: "https://example.com/restaurant14.jpg",
    name: "Thai Orchid",
    rating: 4.4,
    city: "Bangkok",
    typeOfFood: "Thai",
    description:
      "Indulge in the aromatic and spicy flavors of Thailand at Thai Orchid. Our chefs bring the authenticity of Thai cuisine to your table, offering a true taste of Bangkok.",
  },
  {
    imageUrl: "https://example.com/restaurant15.jpg",
    name: "Spanish Tapas Lounge",
    rating: 4.3,
    city: "Barcelona",
    typeOfFood: "Spanish",
    description:
      "Experience the lively and social atmosphere of Spain at Spanish Tapas Lounge. Savor a variety of delicious tapas and immerse yourself in the rich culinary heritage of Barcelona.",
  },
];
